...
export function createGateHook(
  ...
  return async (input, output) => {
    ...
    // FIX: More specific error detection to prevent false positives
    if (currentGate === 'verify') {
      ...
      // Check for actual failure indicators (not just the word "failure")
...
function checkGateAdvance(
  ...
  // Auto-advance BUILD→TEST when implementation files with actual code are written
  // FIX: Require implementation file + actual code content to prevent premature advance
  // ============================================================
  ...
  if (currentGate === 'test') {
    ...
    // Pattern 2: Specific verification commands (REMOVED ls, node - too broad)
    // FIX: Only advance on specific verification actions, not general commands
    const verifyCommands = [
      /^curl\s+(http|https):/,  // URL fetch verification
      /playwright.*test/i,
  ...
  if (currentGate === 'audit') {
    ...
    // Pattern 1: SAST/security scan output (must have clean results)
    const sastPatterns = [
