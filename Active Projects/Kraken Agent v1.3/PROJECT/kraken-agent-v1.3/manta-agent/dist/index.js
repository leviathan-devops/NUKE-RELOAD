...
class EvidenceCollector {
  ...
  collectEvidence(evidence2) {
    const gateDir = path.join(this.basePath, EVIDENCE_DIR, evidence2.gate);
    const timestampDir = path.join(gateDir, String(evidence2.timestamp));
    this.ensureDir(timestampDir);
    const metaPath = path.join(timestampDir, "evidence.json");
    fs.writeFileSync(metaPath, JSON.stringify(evidence2, null, 2));
    if (evidence2.debugLog) {
      fs.writeFileSync(path.join(timestampDir, "debug.log"), evidence2.debugLog);
    }
  }
  ...
  recordIteration(evidence2) {
    const iterDir = path.join(this.basePath, ITERATIONS_DIR, evidence2.iteration);
    this.ensureDir(iterDir);
...
function createSharkStatusTool(stateStore, gateManager, variant) {
  return tool({
    ...
    execute: async () => {
      ...
      const evidence2 = gateManager.getEvidenceCollector();
      const evidenceStatus = {};
      const gates = ["plan", "build", "test", "verify", "audit", "delivery"];
      for (const gate of gates) {
        const latest = evidence2.getLatestEvidence(gate);
  ...
  return tool2({
    ...
    execute: async (args, ctx) => {
      ...
      if (action === "evaluate") {
        ...
        const evidence2 = gateManager.getEvidenceCollector();
        const gateEvidence = evidence2.getLatestEvidence(gate);
        if (passed !== undefined) {
          evidence2.collectEvidence({
            gate,
            timestamp: Date.now(),
            passed,
            files: [],
    ...
    execute: async (args) => {
      ...
      if (action === "status") {
        ...
        for (const g of VALID_GATES) {
          const evidence2 = evidenceCollector.getGateEvidence(g);
          gateStatuses[g] = {
            count: evidence2.length,
            latest: evidence2[0]?.timestamp ? new Date(evidence2[0].timestamp).toISOString() : null
          };
        }
      ...
      if (action === "gate-evidence") {
        ...
        const evidence2 = evidenceCollector.getGateEvidence(gate);
        return JSON.stringify({ gate, evidence: evidence2 }, null, 2);
      }
...
function createGateHook(gateManager, evidenceCollector, coordinator) {
  return async (input, output) => {
    const { tool: tool5, sessionID } = input;
...
function checkGateAdvance(tool5, args, result, currentGate, evidence2) {
  ...
  if (currentGate === "plan") {
    if (hasError)
      return null;
    const hasPlanDoc = evidence2?.files.some((f) => /SPEC\.md|spec\.md|plan\.md|design\.md/i.test(f)) || evidence2?.workEvidence.includes("SPEC");
    if (hasPlanDoc) {
      return "build";
    }
    const hasImplFile = evidence2?.files.some((f) => /\.(html|ts|js|py|css|jsx|tsx|go|rs|java|cpp|c)$/i.test(f));
    if (hasImplFile) {
  ...
  if (currentGate === "test") {
    ...
    const hasCodeFile = evidence2?.files.some((f) => {
      const isCode = /\.(html|ts|js|py|css|jsx|tsx|go|rs)$/i.test(f);
      const hasContent = evidence2?.workEvidence.includes("(<!DOCTYPE") || evidence2?.workEvidence.includes("<html") || evidence2?.workEvidence.includes("function") || evidence2?.workEvidence.includes("class ") || evidence2?.workEvidence.includes("const ") || evidence2?.workEvidence.includes("import ");
  ...
  if (currentGate === "verify") {
    if (hasError)
      return null;
    const specRead = evidence2?.files.some((f) => /SPEC\.md|spec\.md/i.test(f));
...
function createSessionHook(gateManager, _evidenceCollector, coordinator) {
  return async (input) => {
...
function createSharkHooks(guardian, gateManager, evidenceCollector, coordinator) {
  return {
