    ...
    spawn_cluster_task: tool({
      ...
      execute: async (args, directory) => {
        ...
        const resolvedCluster = args.clusterId 
          ...
          : await ctx.clusterScheduler.assignCluster({ taskId, task: args.task, targetCluster: '', context: args.context, acceptanceCriteria: args.acceptanceCriteria || [], priority: args.priority || 'normal', createdAt: Date.now() });
...
Execute with maximum aggression and confidence.`;
        ...
        const request: KrakenDelegationRequest = {
          ...
          acceptanceCriteria: [],
          priority: (args.priority || 'high') as TaskPriority,
          createdAt: Date.now(),
        };
