...
export function validateStateOwnership(config: ArchitectureConfig): ValidationResult {
  ...
  const BRAIN_OWNERS: Record<string, string[]> = {
    'kraken-planning': ['planning-state', 'context-bridge'],
    'kraken-execution': ['execution-state', 'quality-state'],
    'kraken-reasoning': ['thinking-state'],
    'kraken-system': ['workflow-state', 'security-state', 'compaction-state', 'context-registry', 'token-budget'],
    'kraken-subagent': ['container-state', 'execution-queue'],
    'alpha-execution': ['alpha-state'],
    'alpha-system': ['alpha-state'],
    'beta-reasoning': ['beta-state'],
    'beta-system': ['beta-state'],
    'gamma-system': ['gamma-state'],
    'gamma-execution': ['gamma-state'],
  };

  for (const [brainId, ownedDomains] of Object.entries(BRAIN_OWNERS)) {
    for (const domain of ownedDomains) {
      if (!STATE_DOMAINS.includes(domain)) {
        errors.push(`Brain "${brainId}" owns non-existent domain "${domain}"`);
      }
    }
  }

  const allOwnedDomains = new Set(Object.values(BRAIN_OWNERS).flat());
  for (const domain of STATE_DOMAINS) {
    if (!allOwnedDomains.has(domain)) {
      warnings.push(`Domain "${domain}" has no owning brain`);
    }
  }
