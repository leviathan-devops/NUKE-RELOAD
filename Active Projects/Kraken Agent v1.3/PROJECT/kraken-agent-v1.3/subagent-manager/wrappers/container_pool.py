...
def cleanup_orphan_containers() -> int:
    """Clean up orphaned kraken-oc-* containers from previous runs."""
    killed = 0
...
def list_pool_containers() -> list[dict]:
    """List all kraken-oc-pool-* containers."""
    r = subprocess.run(
    ...
    for line in r.stdout.strip().split("\n"):
        ...
        if name.startswith(POOL_PREFIX):
            ...
            port = (
                int(port_str.split("->")[0].split(":")[-1]) if "->" in port_str else 0
            )
    ...
    def run_all(self, tasks: list[dict], timeout: int = 180) -> list[dict]:
        ...
        for wave_idx in range(0, len(tasks), pool_size):
            ...
            with ThreadPoolExecutor(max_workers=len(wave_tasks)) as ex:
                futures = {
                    ex.submit(run_single, j, t): (wave_idx + j)
...
def main():
    ...
    parser.add_argument(
        "--create", "-c", type=int, metavar="N", help="Start N containers in the pool"
    )
    parser.add_argument(
        "--list", "-l", action="store_true", help="List running pool containers"
    )
    parser.add_argument(
        "--kill-all", action="store_true", help="Kill all pool containers"
    )
    parser.add_argument(
        "--pool-size", type=int, default=3, help="Pool size for --create (default: 3)"
    )
